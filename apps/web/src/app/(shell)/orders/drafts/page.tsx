...
import { QC_STATE_LABELS, QC_STATE_TONES, draftTotal, type DraftItem, type OrderDraft } from "@/lib/domain/order-qc";
import { confirmOrderDraft, discardOrderDraft, fetchOrderDrafts, fetchWooConnections, saveOrderDraft, type LiveWooConnection } from "@/lib/supabase/live";
...
function storeLabel(c: LiveWooConnection | undefined): string {
...
function DraftDialog({ draft, connections, onClose, onSaved }: { draft: OrderDraft | null; connections: LiveWooConnection[]; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{draft ? `Edit draft #${draft.id}` : "New draft"}</DialogTitle>
