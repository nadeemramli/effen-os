...
const REGISTRY: AutomationEntry[] = [
  ...
  },
  {
    key: "fulfilment_gate",
    dept: "Logistics & Fulfilment",
    name: "Fulfilment pipeline gate (Synovil MY pilot)",
    what: "Every pilot-store processing order gets a pipeline row: graded by the ship gate, auto-passed with a hold window (default 20 min) an admin can freeze, exceptions queued for fixing. Re-grades corrections within 5 minutes — ADR-0006.",
    trigger: "cron · 5 min",
    guardrail: "runs itself",
    source: "supabase · fulfilment_gate_tick()",
    status: "live",
    link: { href: "/fulfilment", label: "Pipeline" },
    health: (h) =>
      `${n(h.fulfilment?.tracked).toLocaleString()} tracked · ${n(h.fulfilment?.exceptions)} exceptions · ${n(h.fulfilment?.held)} held · last tick ${rel(h.fulfilment?.last_tick_at)}`,
  },
  {
    key: "hold_gate",
    dept: "Logistics & Fulfilment",
    name: "Hold before submission",
    what: "The admin's guaranteed intervention: any pre-submission order can be frozen indefinitely and released back into the gate. Both moves are audited.",
    trigger: "on click",
    guardrail: "human-confirm",
    source: "supabase · hold_fulfilment_order()",
    status: "live",
    health: (h) => `${n(h.fulfilment?.held)} currently held`,
  },
  {
    key: "address_ai",
    dept: "Logistics & Fulfilment",
    name: "AI address suggestions (OpenRouter)",
    what: "Drafts a correction for every flagged pilot-store order after each sync — confidence + rationale stored, cost metered, PII sent with provider data-collection off. A human accepts or dismisses in the fix dialog; nothing auto-applies — ADR-0006.",
    trigger: "cron · 15 min",
    guardrail: "suggest-only",
  ...
  },
  {
    key: "nv_shadow",
    dept: "Logistics & Fulfilment",
    name: "NV shadow submission",
    what: "Builds the exact Ninja Van order payload for every green, unheld, past-window pilot order and logs it — no HTTP call leaves the building. Coverage scored against Woo outcomes; ≥99% for 2 weeks is the go-live gate — ADR-0006/0002.",
    trigger: "cron · 15 min",
    guardrail: "runs itself",
    source: "edge function · nv-submit",
    status: "live",
    link: { href: "/fulfilment", label: "Shadow evidence" },
    health: (h) =>
      `${n(h.nv_shadow?.shadow_14d)} payloads 14d · coverage ${h.nv_shadow?.coverage_pct ?? "—"}% · ${n(h.nv_shadow?.open)} open`,
  },
  {
    key: "woo_writeback",
    dept: "Logistics & Fulfilment",
  ...
  },
  {
    key: "nv_consignment",
    dept: "Logistics & Fulfilment",
    name: "NV consignment creation (live mode)",
    what: "The shadow pipeline flipped live per store via set_fulfilment_mode — submits real courier orders. On hold until the shadow exit gate is met and Fighter is scoped down for the pilot brand — ADR-0006/0002.",
    trigger: "on approval",
    guardrail: "human-confirm",
    source: "on hold · shadow evidence pending",
    status: "on hold",
  },
  // ── Data & Integrations ───────────────────────────────────────────────────
  {
