...
### Phase 1 — Governed customer and profit contracts (Supabase, additive)
Files: `supabase/migrations/<applied-version>_customer_lifecycle_contract.sql` (+ follow-ups), `apps/web/src/lib/supabase/live.ts` (fetchers + types), `apps/web/src/lib/domain/lifecycle.ts` (labels/policy types), SQL tests under `supabase/tests/` (new, pgTAP-style plain SQL asserting invariants, run via MCP/CLI).
- `private.customer_lifecycle_policy` (versioned; fields per R3) with one seeded `provisional` v1 row per workspace.
- `private.customer_qualifying_orders` (order-level rule application: `qualifies_lifecycle`, `qualifies_acquisition`, `accepted_at`, `delivered_at`, exclusion reason) — built from `orders_read`, `nv_shipments`, `order_corrections`.
- `private.customer_acquisition_cohort` (R8), `private.customer_lifecycle_state_daily` (R4, refreshed daily by pg_cron after the identity refresh; backfilled from order history), `private.customer_lifecycle_transition` (R5), `private.customer_base_movement_period` (R6, week/month grains).
- `private.acquisition_spend_allocation` and `private.customer_economics_horizon` contracts created with the v1 provisional methods (R9, R10) but published as `coverage: provisional` until D3–D5 are confirmed.
- Serving: `public.live_customer_base_movement(p_grain, p_from, p_to, p_brand_id, p_integration_ids, p_policy_version, p_lens)` and `public.live_customer_transition_population(...)` (cursor-paginated, masked, `customers.view`), both `security invoker`, explicit grants, envelope per R11. Indexes on scope + period + state.
- Tests: reconciliation identity, uniqueness per customer-day, no impossible transitions, scope isolation, accepted-vs-delivered denominators, zero-opening rate, currency match, empty periods.
- Warehouse note: the source asks for BigQuery/dbt ownership of history. The checked-in dbt project only carries Meta ads today and the customer facts live in Supabase; Phase 1 therefore builds the governed models in Postgres (`private` schema, versioned, tested) and records the dbt migration as a later, owner-approved move rather than blocking on it.
...
|---|---|---|---|
| 2026-08-25 | Phase 0 — audit, doc sync, execution map | (this commit) | Done |
