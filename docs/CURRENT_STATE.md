...
updated: 2026-08-25
status: living
source_commit: 5ee65a4
tags: [fullkit, status, implementation, operations]
---

# Fullkit current development state

This is the release-status companion to the product and architecture documents. It describes what exists on `main` at commit `5ee65a4` (25 Aug 2026, end of the operational-workspaces program Phases 0–7; the Phase 8 hardening commit follows it). When this document conflicts with a proposal, the implementation hierarchy at the end of this page wins.
...
| Marketing | Live | Signed-in sessions never see seeded numbers - an empty warehouse or a failed fetch renders an explicit empty or error state, and the seeded page exists only for builds without Supabase. Meta account coverage, campaign facts, spend, Fullkit revenue/orders, custom date ranges, brand/market/platform scope, CM2, and CM3. | Platform attribution is not incrementality. Google and TikTok appear as unconnected placeholders. |
...
| Setup | Live admin | Authenticated Woo connection management and manual sync, live brand/product/variant setup, Ninja Van API credentials and webhook URL (booking stays shadow), WhatsApp Cloud API credentials and number-to-brand mapping, and OpenRouter key rotation. Secrets are write-only from the browser and stored in Vault/edge-function secrets. | HQ-admin/RLS boundaries apply. Ninja Van credentials and external-write promotion are not exposed as ordinary setup actions. |
| Finance | Demo (labelled) | Commission review/release flow and SQL Accounting handoff are represented in seeded state; a live session sees a prototype banner on the page. | Live contribution rules exist in the backend, but the full finance control UI and accounting integrations are not live. |
| Creative and Reports | Demo (labelled) | Implemented prototype screens and interactions; a live session sees a prototype banner on every page. | Governed live read models are still required. |
| Integrations and Data Health | Live read | Settings > Integrations lists the live integration_connections register by category with real-clock freshness, 24h errors, last failure and checkpoint; the detail page shows scopes, configuration and reason-coded sync history (manual WooCommerce sync for Operations/HQ). Data health shows freshness against SLA per source and every non-success run in the last 24 hours. Global search resolves live connections. | No governed data-quality issue register, trust score or reconciliation count exists, and the page says so. Marketplace listings map to variants by hand (map_marketplace_listing) once a mirror exists. |
