...
|---|---|
| Application | 12 sidebar sections (28 section children across Orders, Customers, Fulfilment), 6 settings entries, and 42 App Router page files |
| Operational backend | 74 migration files covering 77 recorded migrations, 7 edge functions, and one plain-SQL invariant test file under `supabase/tests/` |
...
| Customers | Live | Resolved identities, Customer 360, contribution LTV and risk, address-cluster/reseller signals, saved segments, filtered CSV export, VIP / At risk / Shared address as section-sidebar query views, and a **Customer base** page (`/customers/base`): opening / new / reactivated / lapsed / closing / net-rate cards, diverging movement chart, active-base trend, month/week grain with URL range presets, brand + market scope, policy and freshness badges, coverage banner, definitions panel and a masked drill-through sheet. A governed lifecycle contract (policy v1, provisional 60-day lapse) computes qualifying orders, acquisition cohorts, state intervals, transitions and reconciled weekly/monthly base movement nightly in `private.*`, served by `live_customer_base_movement`, `live_customer_transition_population` and `live_customer_lifecycle_states`; the list and detail pages render that served state (no browser date derivation). | Acquisition lens only. The `activity` field used by saved segments is still the unversioned `≤30d` / `≤90d` SQL `CASE`, so a segment's "At risk" and the served "At risk" can disagree until Phase 3 aligns them; value tier / repeat state remain unversioned. Identity merges are not historised, so the movement corrections line is always 0. Export is capped at 20,000 rows and carries a personal-data warning. |
