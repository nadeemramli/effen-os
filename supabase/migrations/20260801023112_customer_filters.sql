-- Restore the original Customers filter set on the live read-model:
-- repeat state (order-count buckets) and value tier (recognized-revenue
-- thresholds, currency-summed) join lifecycle as server-side filters.
-- New signature → drop the old overload so PostgREST stays unambiguous.
drop function if exists public.live_customers(integer, integer, text, bigint, text, text[]);
create function public.live_customers(
  p_page integer default 1,
  p_page_size integer default 50,
  p_search text default '',
  p_brand_id bigint default null,
  p_activity text default null,   -- new | active | at_risk | dormant | provisional
  p_countries text[] default null,
  p_repeat text default null,     -- first_time | repeat | loyal
  p_tier text default null        -- vip | high | mid | low
)
returns table (
  identity_key text,
  display_name text,
...
begin
  ...
  where (p_brand_id is null or c.brand_ids @> array[p_brand_id])
    ...
    and (p_tier is null
         or (p_tier = 'vip'  and c.revenue_total >= 3000)
         or (p_tier = 'high' and c.revenue_total >= 1000 and c.revenue_total < 3000)
         or (p_tier = 'mid'  and c.revenue_total >= 300  and c.revenue_total < 1000)
         or (p_tier = 'low'  and c.revenue_total < 300))
...
revoke execute on function public.live_customers(integer, integer, text, bigint, text, text[], text, text) from public, anon;
grant execute on function public.live_customers(integer, integer, text, bigint, text, text[], text, text) to authenticated;
